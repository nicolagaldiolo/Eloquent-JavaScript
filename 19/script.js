...
controls.tool = function(cx) {
  ...
  for (var name in tools){
    if(name == "Flood fill"){
      select.appendChild(elt("option", {selected:""}, name));
    }else{
...
function coloraVicino(f){
  f()
  console.log("coloro il vicino");
}

function controllaStessoColore(pos1, pos2, data){
...
tools["Flood fill"] = function(event, cx) {
  ...
  var dataImage = cx.getImageData(0,0,cx.canvas.width, cx.canvas.height);
  
  var alreadyFilled = new Array(dataImage.width * dataImage.height);
  
  var workList = [startPos];
  while(workList.length){
    var pos = workList.pop();
    var offset = pos.x + dataImage.width * pos.y;
    if(alreadyFilled[offset]){
      continue;
    }else{
      cx.fillRect(pos.x, pos.y, 1, 1);
      console.log(controllaStessoColore(pos, pos, dataImage));
      alreadyFilled[offset] = true;

      coloraVicino(function(vicino){
